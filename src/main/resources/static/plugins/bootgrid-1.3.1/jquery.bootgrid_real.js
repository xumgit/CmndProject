...
;(function ($, window, undefined)
{
    ...
    function init()
    {
        ...
        hideColumns.call(this); // for hide thead @20160216        
    ...
    function hideColumns(){
    	var that = this;
    	var arr = that.options.columnNeedRemoved;
    	// TODO hide column in columnNeedRemoved array
    	for (var i=0;i<arr.length;i++) {
    		 for(var j=0;j<this.columns.length;j++){
    			 if(this.columns[j].id==arr[i]){
    				 this.columns[j].visible = false;
    			 }
	        }
    		this.element.find("thead > tr > th[data-column-id^='"+arr[i]+"']").attr("data-visible","false");
    		// this.element.find("thead > tr > th[data-column-id^='"+arr[i]+"']").attr("data-visible-in-selection","true");
    	}
    }
    ...
    */
    ...
    function renderTableHeader()
    {
        ...
        saveInvisibleColumns(this, invisibleColumns); // for test
    ...
    function saveInvisibleColumns(that, arr){
    	if(that.options.callbackFunction && arr && arr.length>0)
    		that.options.callbackFunction(arr);
    }
    ...
    Grid.defaults = {
         ...
         **/
        templates: {
            actionButton: "<button class=\"btn btn-default\" style=\"min-height:34px;\" type=\"button\" title=\"{{ctx.text}}\">{{ctx.content}}</button>",
            ...
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\" style=\"width:auto;padding-left:0px;float:right;padding-right:0px;\"><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>",
    ...
    Grid.prototype.select = function(rowIds)
    {
        if (this.selection)
        {
            ...
            if (selectedRows.length > 0)
            {
                ...
                var  selectedRowLength = this.selectedRows.length;
                
                if(selectedRowLength > 0 && !selectMultiSelectBox){
                	 this.element.find("thead " + selectBoxSelector).prop("checked", false).prop("indeterminate", true);
                }else{//selectMultiSelectBox = true
                	 this.element.find("thead " + selectBoxSelector).prop("indeterminate", false).prop("checked", true);
                }
        ...
        if (this.selection)
        {
            ...
            if (deselectedRows.length > 0)
            {
                ...
                if(selectedRowLength > 0 ){
               	 	this.element.find("thead " + selectBoxSelector).prop("checked", false).prop("indeterminate", true);
                }else{// selectedRowLength = 0
               	 	this.element.find("thead " + selectBoxSelector).prop("indeterminate", false).prop("checked", false);
                }
     ...
     * This method was added by CEAG and is not part of the original code.
     * 
     * @method setRowCount
     * @param [count] {Number} The row count per page to be set.
     * @since 08.03.2016
     */
    Grid.prototype.setRowCount = function (count) {
        this.rowCount = count;
    };
    ...
    /**
     * Sets the actual search phrase.
     * This method was added by CEAG and is not part of the original code.
     * 
     * @method setSearchPhrase
     * @param [phrase] {String} The search phrase to be set.
     * @since 08.03.2016
     */
    Grid.prototype.setSearchPhrase = function (phrase)
    {
        this.searchPhrase = phrase;
    };

    /**
     * Sets the actual search phrase in the textbox.
     * This method was added by CEAG and is not part of the original code.
     * 
     * @method setSearchBoxText
     * @param [phrase] {String} The search phrase to be set in the textbox.
     * @since 08.03.2016
     */
    Grid.prototype.setSearchBoxText = function (phrase) {
        /*var selector = getCssSelector(this.options.css.searchField),
            searchFields = $(selector);
        searchFields.val(phrase);*/
    	$(this.header).find(".search-field").val(phrase);
    };
